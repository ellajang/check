...
const recipientSchema = yup.object().shape({
  ...
  legalPersonName: yup.string().required('This field is mandatory'),
...
export const StepRecipientDetails = ({ handleBack, handleNext }) => {
  ...
  const { countries, banks, banksError, branch, branchError, accountTypes, loading } = useSelector(
  ...
  console.log('bankWatcher', bankWatcher)
              ...
              <Controller
                ...
                render={({ field: { value, onChange } }) => (
                  <CustomTextField
                    ...
                    value={value}
                    onChange={onChange}
                    error={Boolean(errors.legalPersonName)}
                    placeholder={t('Legal Person Name')}
                    {...(errors.legalPersonName && { helperText: t(errors.legalPersonName.message) })}
                  />
                )}
              />
            </Grid>
              ...
              render={({ field: { value, onChange } }) => (
                <CustomTextField
                  ...
                  SelectProps={{
                    value: value,
                    onChange: e => {
                      console.log('on bank change', e.target.value)
                      const selectedBankCode = e.target.value
                      const selectedBank = banks?.filter(bank => bank.bank_code === selectedBankCode)
                      console.log('selected Bank', selectedBank)
                      setValue('swiftBIC', selectedBank[0].swift_code)
                      dispatch(
                        getBankBranch({
                          function: 'LIST',
                          scope: 'BYKEYWORD',
                          data: {
                            query_params: {
                              value: selectedBank[0].bank_code
                            }
                          }
                        })
                      )
                  ...
                  sx={{
                    '& .MuiSelect-select .notranslate::after': 'Select Bank'
                      ? {
                          content: `"${t('Select Bank')}"`,
                          opacity: 0.42
                        }
                      : {}
                  }}
