...
const data = [
  {
    title: 'Bank Deposit (Virtual Account)',
    value: 'BANK_DEPOSIT',
    disabled: false
  },
  {
...
export const StepLocalCollection = ({ handleBack, handleNext }) => {
  ...
  const { createTransaction, depositMethod, transactionId, virtualAccountResponse } = useSelector(
    state => state.payment
  )
    ...
    <Fragment>
      <form onSubmit={handleSubmit(onSubmit)}>
        <Grid container spacing={4}>
          <GMETitle title='Local Collection' subTitle='Please choose the fund deposit method' />
