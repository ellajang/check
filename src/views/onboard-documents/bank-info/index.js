...
const BankInfoView = () => {
  ...
  const renderData = () => {
    return bankInfoData.map((item, index) => {
      return <IdentityCard secLink={'/onboarding/documents/bank-info/edit/'} key={index} title={item.title} subTitle={item.subTitle} avatarIcon={item.avatarIcon} />
    })
  }
  ...
  const renderBankPassbook = () => (
    <IdentityCard
      title='Bank Passbook'
      subTitle='Bank Passbook Document'
