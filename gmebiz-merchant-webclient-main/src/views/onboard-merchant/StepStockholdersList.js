...
import { resetListOfStockholders, updateListOfStockholders } from 'src/store/apps/onboarding'
import FormModalStock from './FormModalStock'
// import { getAllCountries, getAllDesignations } from 'src/store/apps/country'

const StepStockholdersList = ({ handleNext, handleBack }) => {
  ...
  const handleDelete = id => {
    console.log('id', id)
    const updateData = listOfStockholders.filter(data => data.id !== id)
    console.log('dat', updateData)
    dispatch(updateListOfStockholders(updateData))
  }
    ...
    <Fragment>
      <Grid container spacing={4}>
        ...
        <Grid item xs={12} sm={12}>
          {listOfStockholders?.length > 0 &&
            listOfStockholders.map((rep, index) => {
              return (
                <Grid
                  container
                  key={index}
        ...
        >
          ...
          {show && <FormModalStock editData={editData} show={show} handleShow={handleShow} />}
        </Grid>
      </Grid>
...
export default StepStockholdersList
