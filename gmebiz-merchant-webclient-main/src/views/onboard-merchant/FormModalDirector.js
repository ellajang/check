...
import { updateListOfDirectors, resetCompanyRepresentativeFile } from 'src/store/apps/onboarding'
...
const FormModalDirector = ({ editData, show, handleShow }) => {
  ...
  const onSubmit = data => {
    ...
    if (editData) {
      const updateData = listOfDirectors.filter(data => data.id !== editData.id)
      dispatch(updateListOfDirectors([...updateData, data]))
    } else {
      dispatch(updateListOfDirectors([...listOfDirectors, data]))
    }
    handleShow()
  }
