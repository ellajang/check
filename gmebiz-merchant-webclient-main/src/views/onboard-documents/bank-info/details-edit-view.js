...
const bankInfoSchema = yup.object().shape({
  ...
  ifscCode: yup
    .string()
    // .matches(/^[a-zA-Z0-9]+$/g, 'Please Enter alpha numeric value only.')
    .max(15, 'Please enter a maximum of 15 characters.'),
  cardNumber: yup
    .string()
    // .matches(/^[a-zA-Z0-9]+$/g, 'Please Enter alpha numeric value only.')
...
const DetailsEditView = () => {
  ...
  const { merchantBanks, error, loading, message, responseStatus, messageCode } = useSelector(state => state.onboarding)
