...
export const StepTransferConfirmation = ({ handleBack, handleNext }) => {
  ...
  useEffect(() => {
    ...
    if (confirmTransactionResponse?.status === 'SUCCESS') {
      ...
      toast.error(  underScoreTextRemove(confirmTransactionResponse?.transaction_request_details?.transaction_request_response_message) || confirmTransactionResponse?.message || "Something is not Right!")
      dispatch(resetToast())
    }
  ...
  return (
    <Fragment>
      ...
      <Grid container spacing={4}>
        ...
        <Grid item xs={12} sm={12}>
          <Box sx={{ display: 'flex', flexDirection: 'column' }}>
            ...
            >
              ...
              <Typography sx={{ color: 'text.primary' }}>{formatMoney(createTransaction?.serviceFee)}</Typography>
            </Box>
            <Box
              sx={{
                mb: 4,
        ...
        <Grid item xs={12} sm={12}>
          <Typography variant='h5'>
            <Translations text='Sender Details' />{' '}
          </Typography>
        </Grid>
        <Grid item xs={12} sm={12}>
          <Box sx={{ display: 'flex', flexDirection: 'column' }}>
            ...
            >
              <Typography sx={{ color: 'text.secondary' }}>
                <Translations text={`Applicant Name`} />
              </Typography>
