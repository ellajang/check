...
const StepRequestPlaced = ({ handleNext, handlePrev, formData, setFormData }) => {
  ...
  const {transactionId} = useSelector(state=>state.payment)
  ...
  return (
    <>
        ...
        <Grid item xs={12}>
          ...
          <Box sx={{ mb: 6 }}>
            <Typography variant='titleGME' sx={{ mb: 2 }}>
              <Translations text='Request Submitted' />
            </Typography>
            <Box sx={{ mb: 1.5, fontSize: '16px', fontWeight: 400, display: 'flex', alignItems: 'center' }}>
              ...
              <Typography sx={{ ml: '8px' }} display='inline'>
                <Translations text='Time Submitted' />: {format(new Date(), "yyyy-MM-dd ',' HH:mm a")}{' '}
              </Typography>
            </Box>
          </Box>
          {/* </Grid> */}
          ...
          <Grid item xs={12} sm={12}>
            <Typography>
              <Translations text='Your Virtual Account Request is under review. You will be notified you for any action taken on your request via email or SMS.' />
            </Typography>
          </Grid>
        </Grid>
      </Grid>
      <Grid item xs={12} sm={12} sx={{ display: 'flex', columnGap: 6, justifyContent: 'center', alignItems: 'center' }}>
        <Button
          onClick={()=>router.push(`/dashboard/receipt/${transactionId}`)}
          // type='submit'
          variant='outlined'
          sx={{ color: 'text.primary', borderColor: 'text.secondary', mb: 4, mt: 4 }}
        >
          <Translations text='View Receipt' />
        </Button>
